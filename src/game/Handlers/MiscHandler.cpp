...
void WorldSession::HandleRepopRequestOpcode(WorldPacket& /*recv_data*/)
{
    ...
    {
        sLog.outDebug("HandleRepopRequestOpcode: got request after player %s(%d) was killed and before he was updated", player->GetName(), player->GetGUIDLow());
        player->KillPlayer();
    }
...
void WorldSession::HandleWhoOpcode(WorldPacket& recv_data)
{
    ...
    for (uint32 i = 0; i < task.zones_count; ++i)
    {
        ...
        task.zoneids[i] = temp;
        sLog.outDebug("Zone %u: %u", i, task.zoneids[i]);
    }
    ...
    sLog.outDebug("Minlvl %u, maxlvl %u, name %s, guild %s, racemask %u, classmask %u, zones %u, strings %u", task.level_min, task.level_max, player_name.c_str(), guild_name.c_str(), task.racemask, task.classmask, task.zones_count, task.str_count);
...
{
    sLog.outDebug("WORLD: Recvd MSG_LOOKING_FOR_GROUP Message");
    ...
    if (friendGuid)
    {
        ...
        else
        {
            ...
            if (!GetMasterPlayer()->GetSocial()->AddToSocialList(friendGuid, false))
            {
                ...
                sLog.outDebug("WORLD: %s's friend list is full.", GetMasterPlayer()->GetName());
            }
        }
    }
...
void WorldSession::HandleAreaTriggerOpcode(WorldPacket& recv_data)
{
    ...
    uint32 triggerId;
    recv_data >> triggerId;
    sLog.outDebug("Trigger ID: %u", triggerId);
    ...
    {
        sLog.outDebug("Player '%s' (GUID: %u) in flight, ignore Area Trigger ID: %u", pPlayer->GetName(), pPlayer->GetGUIDLow(), triggerId);
        return;
    }
    ...
    if (!pTrigger)
    {
        sLog.outDebug("Player '%s' (GUID: %u) send unknown (by DBC) Area Trigger ID: %u", pPlayer->GetName(), pPlayer->GetGUIDLow(), triggerId);
        return;
    }
    ...
    {
        sLog.outDebug("Player '%s' (GUID: %u) too far, ignore Area Trigger ID: %u", pPlayer->GetName(), pPlayer->GetGUIDLow(), triggerId);
        return;
    }
    ...
    else
    {
        switch (type)
        {
            ...
            case ACTION_BUTTON_ITEM:
                break;
            default:
                sLog.outDetail("MISC: Unknown action button type %u for action %u into button %u", type, action, button);
                return;
        }
...
void WorldSession::HandleInspectOpcode(WorldPacket& recv_data)
{
    ObjectGuid guid;
    recv_data >> guid;
    sLog.outDebug("Inspected guid is %s", guid.GetString().c_str());
...
void WorldSession::HandleWorldTeleportOpcode(WorldPacket& recv_data)
{
    ...
    sLog.outDebug("Time %u sec, map=%u, x=%f, y=%f, z=%f, orient=%f", time / 1000, mapId, positionX, positionY, positionZ, orientation);
...
{
    sLog.outDebug("WORLD: Received opcode CMSG_MOVE_SET_RAW_POSITION from %s", GetPlayer()->GetGuidStr().c_str());
    ...
    //sLog.outDebug("Set to: X=%f, Y=%f, Z=%f, orient=%f", PosX, PosY, PosZ, PosO);

    if (!GetPlayer()->IsInWorld() || GetPlayer()->IsTaxiFlying())
    {
        sLog.outDebug("Player '%s' (GUID: %u) in a transfer, ignore setrawpos command.", GetPlayer()->GetName(), GetPlayer()->GetGUIDLow());
        return;
    }
...
void WorldSession::HandleFarSightOpcode(WorldPacket& recv_data)
{
    ...
    {
        case 0:
            sLog.outDebug("Removed FarSight from %s", _player->GetGuidStr().c_str());
            _player->GetCamera().ResetView(false);
            break;
        case 1:
            sLog.outDebug("Added FarSight %s to %s", _player->GetFarSightGuid().GetString().c_str(), _player->GetGuidStr().c_str());
