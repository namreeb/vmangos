...
bool Map::ScriptCommand_Talk(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    if (!pSource)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_TALK (script id %u) call for a nullptr or non-worldobject source (TypeId: %u), skipping.", script.id, source ? source->GetTypeId() : 0);
...
bool Map::ScriptCommand_Emote(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    if (!pSource)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_EMOTE (script id %u) call for a nullptr or non-worldobject source (TypeId: %u), skipping.", script.id, source ? source->GetTypeId() : 0);
        return ShouldAbortScript(script);
    }
    ...
    if (script.emote.isTargeted)
    {
        ...
        if (!pCreatureSource)
        {
            sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_EMOTE (script id %u) call for targeted emote with non-creature source (TypeId: %u), skipping.", script.id, source ? source->GetTypeId() : 0);
            return ShouldAbortScript(script);
        }
        ...
        if (!pTarget)
        {
            sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_EMOTE (script id %u) call for targeted emote with non-unit target (TypeId: %u), skipping.", script.id, target ? target->GetTypeId() : 0);
            return ShouldAbortScript(script);
        }
...
bool Map::ScriptCommand_FieldSet(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    if (!source)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_FIELD_SET (script id %u) call for a nullptr object, skipping.", script.id);
        return ShouldAbortScript(script);
    }

    if (script.setField.fieldId <= OBJECT_FIELD_ENTRY || script.setField.fieldId >= source->GetValuesCount())
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_FIELD_SET (script id %u) call for wrong field %u (max count: %u) in object (TypeId: %u).",
...
bool Map::ScriptCommand_MoveTo(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    if (!pSource)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_MOVE_TO (script id %u) call for a nullptr or non-creature source (TypeId: %u), skipping.", script.id, source ? source->GetTypeId() : 0);
        return ShouldAbortScript(script);
    }
    ...
    switch (script.moveTo.coordinatesType)
    {
        case SO_MOVETO_COORDINATES_RELATIVE_TO_TARGET:
        {
            ...
            {
                sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_MOVE_TO (script id %u) call with datalong = %u for a nullptr or non-worldobject target, skipping.", script.id, script.moveTo.coordinatesType);
                return ShouldAbortScript(script);
            }
            break;
        }
...
bool Map::ScriptCommand_ModifyFlags(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    if (!source)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_MODIFY_FLAGS (script id %u) call for a nullptr object, skipping.", script.id);
        return ShouldAbortScript(script);
    }

    if (script.modFlags.fieldId <= OBJECT_FIELD_ENTRY || script.modFlags.fieldId >= source->GetValuesCount())
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_MODIFY_FLAGS (script id %u) call for wrong field %u (max count: %u) in object (TypeId: %u).",
...
bool Map::ScriptCommand_InterruptCasts(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    if (!pSource)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_INTERRUPT_CAST (script id %u) call for a nullptr or non-unit source (TypeId: %u), skipping.", script.id, source ? source->GetTypeId() : 0);
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_TeleportTo(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    if (!pSource)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_TELEPORT_TO (script id %u) call for a nullptr or non-unit source (TypeId: %u), skipping.", script.id, source ? source->GetTypeId() : 0);
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_QuestExplored(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_QUEST_EXPLORED (script id %u) call for a nullptr player, skipping.", script.id);
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_KillCredit(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_KILL_CREDIT (script id %u) call for a nullptr object, skipping.", script.id);
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_RespawnGameObject(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    if (!pGo)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_RESPAWN_GAMEOBJECT (script id %u) failed for gameobject (guid: %u).", script.id, guidlow);
        return ShouldAbortScript(script);
    }
    ...
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_RESPAWN_GAMEOBJECT (script id %u) can not be used with gameobject of type %u (guid: %u).", script.id, uint32(pGo->GetGoType()), script.respawnGo.goGuid);
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_SummonCreature(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_TEMP_SUMMON_CREATURE (script id %u) call for a nullptr creature, skipping.", script.id);
        return ShouldAbortScript(script);
    }
    ...
    if (!pSummoner)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_TEMP_SUMMON_CREATURE (script id %u) call for a nullptr or non-worldobject source (TypeId: %u, skipping.", script.id, source ? source->GetTypeId() : 0);
        return ShouldAbortScript(script);
    }
    ...
    if (!pCreature)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_TEMP_SUMMON (script id %u) failed for creature (entry: %u).", script.id, script.summonCreature.creatureEntry);
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_OpenDoor(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    if (!pDoor)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_OPEN_DOOR (script id %u) failed for gameobject (guid: %u).", script.id, guidlow);
        return ShouldAbortScript(script);
    }

    if (pDoor->GetGoType() != GAMEOBJECT_TYPE_DOOR)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_OPEN_DOOR (script id %u) failed for non-door(GoType: %u).", script.id, pDoor->GetGoType());
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_CloseDoor(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    if (!pDoor)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_CLOSE_DOOR (script id %u) failed for gameobject (guid: %u).", script.id, guidlow);
        return ShouldAbortScript(script);
    }
    if (pDoor->GetGoType() != GAMEOBJECT_TYPE_DOOR)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_CLOSE_DOOR (script id %u) failed for non-door(GoType: %u).", script.id, pDoor->GetGoType());
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_ActivateGameObject(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_ACTIVATE_OBJECT (script id %u) call for a nullptr user, skipping.", script.id);
        return ShouldAbortScript(script);
    }

    if (!((pGo = ToGameObject(target)) || (pGo = ToGameObject(source))))
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_ACTIVATE_OBJECT (script id %u) call for a nullptr gameobject, skipping.", script.id);
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_RemoveAura(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    if (!pSource)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_REMOVE_AURA (script id %u) call for a nullptr or non-unit source (TypeId: %u), skipping.", script.id, source ? source->GetTypeId() : 0);
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_CastSpell(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    if (!source)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_CAST_SPELL (script id %u) call for a nullptr source, skipping.", script.id);
        return ShouldAbortScript(script);
    }
    ...
    if (!pSource)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_CAST_SPELL (script id %u) call for a non-unit and non-gameobject source, skipping.", script.id);
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_PlaySound(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    if (!pSource)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_PLAY_SOUND (script id %u) call for a nullptr or non-worldobject source (TypeId: %u), skipping.", script.id, source ? source->GetTypeId() : 0);
        return ShouldAbortScript(script);
    }
    ...
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_PLAY_SOUND (script id %u) in targeted mode call for a nullptr or non-player target (TypeId: %u), skipping.", script.id, target ? target->GetTypeId() : 0);
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_CreateItem(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_CREATE_ITEM (script id %u) call for a nullptr or non-player object (TypeIdSource: %u)(TypeIdTarget: %u), skipping.", script.id, source ? source->GetTypeId() : 0, target ? target->GetTypeId() : 0);
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_DespawnCreature(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    if (!pSource)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_DESPAWN_CREATURE (script id %u) call for a nullptr or non-creature source (TypeId: %u), skipping.", script.id, source ? source->GetTypeId() : 0);
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_SetMovementType(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_MOVEMENT (script id %u) call for a nullptr or non-creature source and target (TypeIdSource: %u)(TypeIdTarget: %u), skipping", script.id, source ? source->GetTypeId() : 0, target ? target->GetTypeId() : 0);
        return ShouldAbortScript(script);
    }
    ...
    switch (script.movement.movementType)
    {
        ...
        case DISTANCING_MOTION_TYPE:
            ...
            break;
        default:
            sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_MOVEMENT (script id %u) call for an invalid motion type (MotionType: %u), skipping.", script.id, script.movement.movementType);
            return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_SetActiveObject(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_SET_ACTIVEOBJECT (script id %u) call for a nullptr or non-creature source and target (TypeIdSource: %u)(TypeIdTarget: %u), skipping.", script.id, source ? source->GetTypeId() : 0, target ? target->GetTypeId() : 0);
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_SetFaction(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    if (!((pSource = ToCreature(source)) || (pSource = ToCreature(target))))
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_SET_FACTION (script id %u) call for a nullptr or non-creature source and target (TypeIdSource: %u)(TypeIdTarget: %u), skipping.", script.id, source ? source->GetTypeId() : 0, target ? target->GetTypeId() : 0);
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_Morph(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_MORPH_TO_ENTRY_OR_MODEL (script id %u) call for a nullptr or non-unit source and target (TypeIdSource: %u)(TypeIdTarget: %u), skipping.", script.id, source ? source->GetTypeId() : 0, target ? target->GetTypeId() : 0);
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_Mount(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    if (!pSource)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_MOUNT_TO_ENTRY_OR_MODEL (script id %u) call for a nullptr or non-unit source and target (TypeIdSource: %u)(TypeIdTarget: %u), skipping.", script.id, source ? source->GetTypeId() : 0, target ? target->GetTypeId() : 0);
        return ShouldAbortScript(script);
    }
    ...
    if (script.mount.permanent)
    {
        ...
        if (!pCreature)
        {
            sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_MOUNT_TO_ENTRY_OR_MODEL (script id %u) call for a nullptr or non-creature source and target (TypeIdSource: %u)(TypeIdTarget: %u), skipping.", script.id, source ? source->GetTypeId() : 0, target ? target->GetTypeId() : 0);
            return ShouldAbortScript(script);
        }
...
bool Map::ScriptCommand_SetRun(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    if (!((pSource = ToCreature(source)) || (pSource = ToCreature(target))))
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_SET_RUN (script id %u) call for a nullptr or non-creature source and target (TypeIdSource: %u)(TypeIdTarget: %u), skipping.", script.id, source ? source->GetTypeId() : 0, target ? target->GetTypeId() : 0);
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_AttackStart(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_ATTACK_START (script id %u) call for a nullptr source or target (TypeIdSource: %u)(TypeIdTarget: %u), skipping.", script.id, source ? source->GetTypeId() : 0, target ? target->GetTypeId() : 0);
        return ShouldAbortScript(script);
    }
    ...
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_ATTACK_START (script id %u) for an invalid attack target, skipping.", script.id);
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_UpdateEntry(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    if (!pSource)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_UPDATE_ENTRY (script id %u) call for a nullptr or non-creature source (TypeId: %u), skipping.", script.id, source ? source->GetTypeId() : 0);
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_SetStandState(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    if (!pSource)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_STAND_STATE (script id %u) call for a nullptr or non-unit source (TypeId: %u), skipping", script.id, source ? source->GetTypeId() : 0);
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_ModifyThreat(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    if (!pSource)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_MODIFY_THREAT (script id %u) call for a nullptr or non-creature source (TypeId: %u), skipping.", script.id, source ? source->GetTypeId() : 0);
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_SendTaxiPath(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    if (!((pPlayer = ToPlayer(target)) || (pPlayer = ToPlayer(source))))
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_SEND_TAXI_PATH (script id %u) call for a non-player source and target (TypeIdSource: %u)(TypeIdTarget: %u), skipping.", script.id, source ? source->GetTypeId() : 0, target ? target->GetTypeId() : 0);
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_TerminateScript(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    if (script.terminateScript.creatureEntry)
    {
        ...
        {
            sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_TERMINATE_SCRIPT (script id %u) call for a nullptr object, skipping.", script.id);
            return ShouldAbortScript(script);
        }
...
bool Map::ScriptCommand_Evade(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    if (!((pSource = ToCreature(source)) || (pSource = ToCreature(target))))
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_ENTER_EVADE_MODE (script id %u) call for a nullptr or non-creature source and target (TypeIdSource: %u)(TypeIdTarget: %u), skipping.", script.id, source ? source->GetTypeId() : 0, target ? target->GetTypeId() : 0);
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_SetHomePosition(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    if (!((pSource = ToCreature(source)) || (pSource = ToCreature(target))))
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_SET_HOME_POSITION (script id %u) call for a nullptr or non-creature source and target (TypeIdSource: %u)(TypeIdTarget: %u), skipping.", script.id, source ? source->GetTypeId() : 0, target ? target->GetTypeId() : 0);
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_TurnTo(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    if (!pSource)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_TURN_TO (script id %u) call for a nullptr or non-unit source (TypeId: %u), skipping.", script.id, source ? source->GetTypeId() : 0);
        return ShouldAbortScript(script);
    }
    ...
    switch (script.turnTo.facingLogic)
    {
        case SO_TURNTO_FACE_TARGET:
        {
            ...
            {
                sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_TURN_TO (script id %u) call with datalong=0 for a non-worldobject target (TypeId: %u), skipping", script.id, target ? target->GetTypeId() : 0);
                return ShouldAbortScript(script);
            }
            break;
        }
...
bool Map::ScriptCommand_MeetingStone(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    if (!((pPlayer = ToPlayer(target)) || (pPlayer = ToPlayer(source))))
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_MEETINGSTONE (script id %u) call for a nullptr or non-player source and target (TypeIdSource: %u)(TypeIdTarget: %u), skipping.", script.id, source ? source->GetTypeId() : 0, target ? target->GetTypeId() : 0);
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_SetData(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    if (!pInst)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_SET_INST_DATA (script id %u) call for map without an instance script, skipping.", script.id);
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_SetData64(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    if (!pInst)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_SET_INST_DATA64 (script id %u) call for map without an instance script, skipping.", script.id);
        return ShouldAbortScript(script);
    }
    ...
    switch (script.setData64.type)
    {
        ...
        case SO_INSTDATA64_SOURCE_GUID:
        {
            ...
            {
                sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_SET_INST_DATA64 (script id %u) call for a nullptr source, skipping.", script.id);
                return ShouldAbortScript(script);
            }
            break;
        }
...
bool Map::ScriptCommand_RemoveItem(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    if (!((pPlayer = ToPlayer(target)) || (pPlayer = ToPlayer(source))))
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_REMOVE_ITEM (script id %u) call for a nullptr or non-player source and target (TypeIdSource: %u)(TypeIdTarget: %u), skipping.", script.id, source ? source->GetTypeId() : 0, target ? target->GetTypeId() : 0);
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_RemoveGameObject(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    if (!((pGo = ToGameObject(target)) || (pGo = ToGameObject(source))))
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_REMOVE_OBJECT (script id %u) call for a nullptr gameobject, skipping.", script.id);
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_SetMeleeAttack(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    if (!pSource)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_SET_MELEE_ATTACK (script id %u) call for a nullptr or non-creature source (TypeId: %u), skipping.", script.id, source ? source->GetTypeId() : 0);
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_SetCombatMovement(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    if (!pSource)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_SET_COMBAT_MOVEMENT (script id %u) call for a nullptr or non-creature source (TypeId: %u), skipping.", script.id, source ? source->GetTypeId() : 0);
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_SetPhase(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    if (!pSource)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_SET_PHASE (script id %u) call for a nullptr or non-creature source (TypeId: %u), skipping.", script.id, source ? source->GetTypeId() : 0);
        return ShouldAbortScript(script);
    }
    ...
    {
        sLog.Out(LOG_DBERROR, LOG_LVL_MINIMAL, "SCRIPT_COMMAND_SET_PHASE (script id %u) attempt to increment Phase above %u. Phase mask cannot be used with phases past %u. CreatureEntry = %d", script.id, MAX_PHASE - 1, MAX_PHASE - 1, pSource->GetEntry());
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_SetPhaseRandom(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    if (!pSource)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_SET_PHASE_RANDOM (script id %u) call for a nullptr or non-creature source (TypeId: %u), skipping.", script.id, source ? source->GetTypeId() : 0);
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_SetPhaseRange(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    if (!pSource)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_SET_PHASE_RANGE (script id %u) call for a nullptr or non-creature source (TypeId: %u), skipping.", script.id, source ? source->GetTypeId() : 0);
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_Flee(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    if (!pSource)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_FLEE (script id %u) call for a nullptr or non-creature source (TypeId: %u), skipping.", script.id, source ? source->GetTypeId() : 0);
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_DealDamage(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    if (!pSource)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_DEAL_DAMAGE (script id %u) call for a nullptr or non-unit source (TypeId: %u), skipping.", script.id, source ? source->GetTypeId() : 0);
        return ShouldAbortScript(script);
    }
    ...
    if (!pTarget)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_DEAL_DAMAGE (script id %u) call for a nullptr or non-unit target (TypeId: %u), skipping.", script.id, target ? target->GetTypeId() : 0);
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_ZoneCombatPulse(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    if (!pSource)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_ZONE_COMBAT_PULSE (script id %u) call for a nullptr or non-creature source (TypeId: %u), skipping.", script.id, source ? source->GetTypeId() : 0);
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_CallForHelp(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    if (!pSource)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_CALL_FOR_HELP (script id %u) call for a nullptr or non-creature source (TypeId: %u), skipping.", script.id, source ? source->GetTypeId() : 0);
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_SetSheath(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    if (!pSource)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_SET_SHEATH (script id %u) call for a nullptr or non-unit source (TypeId: %u), skipping.", script.id, source ? source->GetTypeId() : 0);
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_Invincibility(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    if (!pSource)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_INVINCIBILITY (script id %u) call for a nullptr or non-creature source (TypeId: %u), skipping.", script.id, source ? source->GetTypeId() : 0);
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_CreatureSpells(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    if (!pSource)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_CREATURE_SPELLS (script id %u) call for a nullptr or non-creature source (TypeId: %u), skipping.", script.id, source ? source->GetTypeId() : 0);
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_RemoveGuardians(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    if (!pSource)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_REMOVE_GUARDIANS (script id %u) call for a nullptr or non-unit source (TypeId: %u), skipping.", script.id, source ? source->GetTypeId() : 0);
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_AddSpellCooldown(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    if (!pSource)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_ADD_SPELL_COOLDOWN (script id %u) call for a nullptr or non-unit source (TypeId: %u), skipping.", script.id, source ? source->GetTypeId() : 0);
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_RemoveSpellCooldown(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    if (!pSource)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_REMOVE_SPELL_COOLDOWN (script id %u) call for a nullptr or non-unit source (TypeId: %u), skipping.", script.id, source ? source->GetTypeId() : 0);
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_SetReactState(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    if (!pSource)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_SET_REACT_STATE (script id %u) call for a nullptr or non-creature source (TypeId: %u), skipping.", script.id, source ? source->GetTypeId() : 0);
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_StartWaypoints(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    if (!pSource)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_START_WAYPOINTS (script id %u) call for a nullptr or non-creature source (TypeId: %u), skipping.", script.id, source ? source->GetTypeId() : 0);
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_AddMapEventTarget(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    if (!source)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_ADD_MAP_EVENT_TARGET (script id %u) call for a nullptr source, skipping.", script.id);
        return ShouldAbortScript(script);
    }
    ...
    if (!pEvent)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_ADD_MAP_EVENT_TARGET (script id %u) call for a non-existing scripted map event (EventId: %u), skipping.", script.id, script.addMapEventTarget.eventId);
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_RemoveMapEventTarget(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    if (!pEvent)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_REMOVE_MAP_EVENT_TARGET (script id %u) call for a non-existing scripted map event (EventId: %u), skipping.", script.id, script.removeMapEventTarget.eventId);
        return ShouldAbortScript(script);
    }
    ...
    switch (script.removeMapEventTarget.targets)
    {
        ...
        case SO_REMOVETARGET_ALL_FIT_CONDITION:
        {
            ...
            {
                sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_REMOVE_MAP_EVENT_TARGET (script id %u) call with `datalong3`=%u but without a condition Id, skipping.", script.id, script.removeMapEventTarget.targets);
                return ShouldAbortScript(script);
            }
...
bool Map::ScriptCommand_SetMapEventData(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    if (!pEvent)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_SET_MAP_EVENT_DATA (script id %u) call for a non-existing scripted map event (EventId: %u), skipping.", script.id, script.setMapEventData.eventId);
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_SendMapEvent(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    if (!pEvent)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_SEND_MAP_EVENT (script id %u) call for a non-existing scripted map event (EventId: %u), skipping.", script.id, script.sendMapEvent.eventId);
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_SetDefaultMovement(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    if (!pSource)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_SET_DEFAULT_MOVEMENT (script id %u) call for a nullptr or non-creature source (TypeId: %u), skipping.", script.id, source ? source->GetTypeId() : 0);
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_StartScriptForAll(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    if (!source)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_START_SCRIPT_FOR_ALL (script id %u) call for a nullptr source, skipping.", script.id);
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_EditMapEvent(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    if (!pEvent)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_EDIT_MAP_EVENT (script id %u) call for a non-existing scripted map event (EventId: %u), skipping.", script.id, script.editMapEvent.eventId);
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_FailQuest(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_FAIL_QUEST (script id %u) call for a nullptr object, skipping.", script.id);
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_RespawnCreature(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    if (!pSource)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_RESPAWN_CREATURE (script id %u) call for a nullptr or non-creature source (TypeId: %u), skipping.", script.id, source ? source->GetTypeId() : 0);
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_AssistUnit(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    if (!pSource)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_ASSIST_UNIT (script id %u) call for a nullptr or non-creature source (TypeId: %u), skipping.", script.id, source ? source->GetTypeId() : 0);
        return ShouldAbortScript(script);
    }
    ...
    if (!pTarget)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_ASSIST_UNIT (script id %u) call for a nullptr or non-unit target (TypeId: %u), skipping.", script.id, target ? target->GetTypeId() : 0);
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_CombatStop(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    if (!pSource)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_COMBAT_STOP (script id %u) call for a nullptr or non-unit source (TypeId: %u), skipping.", script.id, source ? source->GetTypeId() : 0);
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_AddAura(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    if (!pSource)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_ADD_AURA (script id %u) call for a nullptr or non-unit source (TypeId: %u), skipping.", script.id, source ? source->GetTypeId() : 0);
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_AddThreat(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    if (!pSource)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_ADD_THREAT (script id %u) call for a nullptr or non-creature source (TypeId: %u), skipping.", script.id, source ? source->GetTypeId() : 0);
        return ShouldAbortScript(script);
    }
    ...
    if (!pTarget)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_ADD_THREAT (script id %u) call for a nullptr or non-unit target (TypeId: %u), skipping.", script.id, target ? target->GetTypeId() : 0);
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_SummonObject(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    if (!source)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_SUMMON_OBJECT (script id %u) call for a nullptr source, skipping.", script.id);
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_SetFly(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    if (!pSource)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_SET_FLY (script id %u) call for a nullptr or non-unit source (TypeId: %u), skipping.", script.id, source ? source->GetTypeId() : 0);
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_JoinCreatureGroup(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    if (!pSource)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_JOIN_CREATURE_GROUP (script id %u) call for a nullptr or non-creature source (TypeId: %u), skipping.", script.id, source ? source->GetTypeId() : 0);
        return ShouldAbortScript(script);
    }
    ...
    if (!pTarget)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_JOIN_CREATURE_GROUP (script id %u) call for a nullptr or non-creature target (TypeId: %u), skipping.", script.id, target ? target->GetTypeId() : 0);
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_LeaveCreatureGroup(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    if (!pSource)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_LEAVE_CREATURE_GROUP (script id %u) call for a nullptr or non-creature source (TypeId: %u), skipping.", script.id, source ? source->GetTypeId() : 0);
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_SetGoState(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    if (!((pGo = ToGameObject(target)) || (pGo = ToGameObject(source))))
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_SET_GO_STATE (script id %u) call for a nullptr gameobject, skipping.", script.id);
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_DespawnGameObject(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    if (!pGo)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_DESPAWN_GAMEOBJECT (script id %u) failed for gameobject (guid: %u).", script.id, guidlow);
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_LoadGameObject(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_LOAD_GAMEOBJECT (script id %u) tried to spawn guid %u on wrong map %u.", script.id, script.loadGo.goGuid, GetId());
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_QuestCredit(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    if (!((pPlayer = ToPlayer(target)) || (pPlayer = ToPlayer(source))))
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_QUEST_CREDIT (script id %u) call for a nullptr player, skipping.", script.id);
        return ShouldAbortScript(script);
    }
    ...
    if (!pWorldObject)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_QUEST_CREDIT (script id %u) call for a nullptr target, skipping.", script.id);
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_SetGossipMenu(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    if (!pSource)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_SET_GOSSIP_MENU (script id %u) call for a nullptr or non-creature source (TypeId: %u), skipping.", script.id, source ? source->GetTypeId() : 0);
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_SendScriptEvent(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    if (!pSource)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_SEND_SCRIPT_EVENT (script id %u) call for a nullptr or non-creature source (TypeId: %u), skipping.", script.id, source ? source->GetTypeId() : 0);
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_SetPvP(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    if (!((pSource = ToPlayer(target)) || (pSource = ToPlayer(source))))
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_SET_PVP (script id %u) call for a nullptr or non-player object (TypeIdSource: %u)(TypeIdTarget: %u), skipping.", script.id, source ? source->GetTypeId() : 0, target ? target->GetTypeId() : 0);
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_ResetDoorOrButton(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    if (!((pGo = ToGameObject(target)) || (pGo = ToGameObject(source))))
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_RESET_DOOR_OR_BUTTON (script id %u) call for a nullptr gameobject, skipping.", script.id);
        return ShouldAbortScript(script);
    }
...
bool Map::ScriptCommand_SetCommandState(ScriptInfo const& script, WorldObject* source, WorldObject* target)
{
    ...
    if (!pSource)
    {
        sLog.Out(LOG_BASIC, LOG_LVL_MINIMAL, "ERROR: SCRIPT_COMMAND_SET_COMMAND_STATE (script id %u) call for a nullptr or non-creature source (TypeId: %u), skipping.", script.id, source ? source->GetTypeId() : 0);
        return ShouldAbortScript(script);
    }
